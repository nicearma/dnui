...
<h3><?php _e("Are the images actually being not used?", 'dnui'); ?></h3>
<p>
    <?php _e("Yes and not, the 'not used' label, tells you that the imageName.imageType (toto.jpg) is not found in any Post/Page and shortcodes, this does not mean a page generated by a plugin is not using this image and therefore if you delete said image be aware that it could be that a plugin is using it.", 'dnui'); ?>
    <br/>
    <?php _e("To go further:", 'dnui'); ?>
    <br/>
    <?php _e("So if you have another plugin, for example 'E-commerce X' that use the example.jpg in one HTML code or Javascript, if the DNUI plugin can't find any reference, it will return a false 'not used' label", 'dnui'); ?>
...
<h3><?php _e("Where i can found the version pro?", 'dnui'); ?></h3>
<p>
    <a href="https://apps.nicearma.com/">DNUI PRO (Delete not used images)</a>
</p>
